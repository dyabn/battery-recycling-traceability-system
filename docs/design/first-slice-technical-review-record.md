...
| SQL 设计无明显语法和约束冲突 | 待 CI 复核 | 已完成静态数量和关键约束检查；已新增 GitHub Actions MySQL 8.4 建表验证，等待执行结果。 |
...
| MySQL 8 实际建表检查 | 待 GitHub Actions 执行；工作流使用 `mysql:8.4` 服务执行 `contracts/database/first-slice-schema-design.sql` 并校验 21 张表 |
| 技术测试数量检查 | 25 个技术测试用例 |
...
| TD-REVIEW-002 | 使用 MySQL 8 或兼容工具解析设计 SQL。 | 已新增 GitHub Actions 验证，待运行结果回填。 |
