...
当前检查点：设计评审  
文档状态：待评审  
输入：C4 第一切片测试设计 V1.0、第一切片 API 设计 V0.1、数据库设计 V0.1
...
| TC-C4-015 | TD-TC-015 | 创建和修改批次接口 | `recycle_batch` | MOD-BATCH |
...
| TD-TC-013 | 已生效记录删除保护 | 已存在验收或入库记录。 | 尝试删除已生效记录。 | 返回 `EFFECTIVE_RECORD_DELETE_FORBIDDEN`；原记录保留；写审计。 | 安全、审计 |
...
| OpenAPI 文件可解析 | 待 TD-TC-020 使用工具验证。 |
