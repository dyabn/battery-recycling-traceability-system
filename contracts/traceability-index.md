...
| FR-C4-003 | UI-C4-004 | `POST /api/v1/recycle-batches`、`PUT /api/v1/recycle-batches/{id}` | `recycle_batch`、`business_attachment` | MOD-BATCH |
| FR-C4-004 | UI-C4-006 | `POST /api/v1/batteries` | `battery`、`lifecycle_event` | MOD-BATTERY、MOD-TRACE |
| FR-C4-005 | UI-C4-006 | `POST /api/v1/batteries` | `battery` | MOD-BATTERY |
| FR-C4-006 | UI-C4-006、UI-C4-014 | `POST /api/v1/batteries`、`POST /api/v1/batteries/duplicate-check` | `battery` | MOD-BATTERY、MOD-DUPLICATE |
| FR-C4-007 | UI-C4-006 | `POST /api/v1/batteries/{id}/duplicate-resolution` | `duplicate_code_review`、`battery` | MOD-DUPLICATE |
...
| FR-C4-013 | UI-C4-009 | `POST /api/v1/batteries/{id}/acceptance-supplements` | `acceptance_supplement`、`battery`、`lifecycle_event` | MOD-ACCEPTANCE、MOD-TRACE |
...
| FR-C4-016 | UI-C4-011 | `POST /api/v1/batteries/{id}/inbounds` | `warehouse`、`warehouse_location`、`audit_log` | MOD-INBOUND、MOD-AUDIT |
...
| FR-C4-021 | UI-C4-001、UI-C4-002 | `POST /api/v1/auth/login`、`GET /api/v1/auth/current-user`、`GET /api/v1/roles`、`GET /api/v1/permissions` | `sys_user`、`sys_role`、`sys_permission`、`sys_user_role`、`sys_role_permission` | MOD-AUTH |
| FR-C4-022 | UI-C4-014 | 删除保护统一拦截 | `acceptance_record`、`inbound_record`、`inventory`、`lifecycle_event`、`audit_log` | MOD-AUDIT |

## 系统设计完整链路摘要

| BSR | UR | FR | AC | TC | UI | API | DB | MODULE |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| BSR-C3-001 | UR-C4-001 | FR-C4-001..003 | AC-001、AC-004 | TC-C4-001、002、015 | UI-C4-003..005 | 批次创建、修改、查询、提交接口 | `recycle_batch`、`business_attachment` | MOD-BATCH |
| BSR-C3-002 | UR-C4-002 | FR-C4-004、005 | AC-002、AC-012 | TC-C4-001、014 | UI-C4-006、013 | 电池登记、追溯接口 | `battery`、`lifecycle_event` | MOD-BATTERY、MOD-TRACE |
| BSR-C3-003 | UR-C4-003 | FR-C4-006..008 | AC-003 | TC-C4-004、005 | UI-C4-006、014 | 重复检查、重复核实、加入批次接口 | `battery`、`duplicate_code_review`、`recycle_batch_battery` | MOD-DUPLICATE、MOD-BATTERY、MOD-BATCH |
...
| BSR-C3-006 | UR-C4-005、006 | FR-C4-012、013、019 | AC-006、AC-007、AC-012 | TC-C4-007、014、016 | UI-C4-008、009、013 | 验收接口、补充资料接口、追溯接口 | `acceptance_record`、`acceptance_supplement`、`battery`、`lifecycle_event` | MOD-ACCEPTANCE、MOD-TRACE |
...
| BSR-C3-008 | UR-C4-007、008 | FR-C4-015..019 | AC-009..012 | TC-C4-001、009、010、011、014 | UI-C4-010..013 | 待入库、入库、库存、追溯接口 | `warehouse`、`warehouse_location`、`inbound_record`、`inventory`、`battery`、`lifecycle_event` | MOD-INBOUND、MOD-INVENTORY、MOD-TRACE |
| BSR-C3-009 | UR-C4-009 | FR-C4-020、021 | AC-013 | TC-C4-012、014、017 | UI-C4-001、002、011、014 | 登录、当前用户、审计、角色、权限接口 | 权限表、`audit_log` | MOD-AUTH、MOD-AUDIT |
| BSR-C3-010 | UR-C4-010 | FR-C4-022 | AC-014 | TC-C4-013 | UI-C4-014 | 删除保护统一拦截 | 生效业务表、`audit_log` | MOD-AUDIT |

## 系统设计覆盖检查

| 检查项 | 结果 |
| --- | --- |
| 没有需求来源的表 | 未发现；19 张表均服务第一切片模块。 |
